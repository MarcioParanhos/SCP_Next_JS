...
// Props:
// - `initialData`: DTO construído server-side (veja src/app/school_units/[id]/page.tsx)
//
// Responsabilidades:
// - Renderizar navegação lateral (tabs) para as views: Informações, Histórico,
//   Homologação e Editar
// - Manter um estado local `data` que pode ser atualizado por callbacks dos
//   formulários filhos (`EditSchoolUnitForm`, `HomologationForm`) para manter
//   a interface responsiva e consistente sem forçar reloads desnecessários.
//
// Observação: o `initialData` já contém os campos de homologação precomputados
// (is_homologated, last_homologation_*) para evitar que o cliente faça uma
// consulta redundante ao carregar a página.
export default function SchoolUnitDetail({ initialData }: { initialData: any }) {
  const [tab, setTab] = React.useState("info");
  const [data, setData] = React.useState(initialData);
  const router = useRouter();
  // Removida lógica de homologação — não usamos mais esses campos aqui.
  return (
    <div className="flex gap-6 p-6">
      ...
      <main className="flex-1 border rounded p-6">
        ...
        {/* Aba de homologação removida conforme solicitado */}
